...
function Init()
{
    ...
    var btn_reserver = document.getElementById("btn_valider_reservation");
    if(btn_reserver != null){
        btn_reserver.addEventListener("click", Reserver, false);
    }
}

function Reserver(){
    var R_nom = document.getElementById("input_nom_reservation").value;
    var R_date = document.getElementById("input_date_reservation").value;
    var R_nbpers = document.getElementById("input_nbpers_reservation").value;
    var newResa = new Reservation(R_nom, R_date, R_nbpers, Status.PasArrive);
    VoyageCulinaire.addReservation(newResa);
    console.log(R_nom + R_date + R_nbpers);
    alert("Vous avez fait une réservation au nom de "+R_nom+" le "+R_date+" pour "+R_nbpers+" personnes");
}
